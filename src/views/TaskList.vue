...
import { boxCountSummary } from '../utils';
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
          <div>
            ...
            <div style="font-size:13px;color:var(--text-secondary);margin-top:4px;">
              {{ t.from }} → {{ t.to }} · {{ t.date }} ·
              <template v-if="boxCountSummary(t).delta !== 0">
                <span style="color:var(--warning);font-weight:700;">{{ boxCountSummary(t).effective }} 箱</span>
                <span>（登记 {{ boxCountSummary(t).registered }}）</span>
              </template>
              <template v-else>{{ t.boxes.length }} 箱</template>
            </div>
          </div>
